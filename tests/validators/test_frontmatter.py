...
name: mycli-shared
description: >
  Shared mycli-cli authorization and notice protocol used by every other di-* skill.
  TRIGGER when: any di-* skill is loaded; the agent needs the di envelope contract.
  DO NOT TRIGGER when: the user is asking a general question with no mycli-cli action involved.
maintainer:
  - owner@example.com
version: 0.1.0
metadata:
...
def test_parses_valid_frontmatter() -> None:
    ...
    assert result.data["name"] == "mycli-shared"
    assert result.body.lstrip().startswith("# mycli-shared")
...
def test_valid_frontmatter_yields_no_issues() -> None:
    ...
    assert validate_frontmatter(data, directory_name="mycli-shared") == []
...
def test_missing_name_is_fail() -> None:
    data = _make(VALID_TEXT)
    del data["name"]
    issues = validate_frontmatter(data, directory_name="mycli-shared")
...
def test_wrong_prefix_is_fail() -> None:
    ...
    assert any("mycli-" in i.message for i in issues if i.status == "fail")


def test_name_mismatch_with_directory_is_fail() -> None:
    data = _make(VALID_TEXT)
    issues = validate_frontmatter(data, directory_name="mycli-other")
    assert any(

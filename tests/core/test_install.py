...
source skills directory (via ``DI_SKILLS_DIR``) so nothing escapes into
...
def source(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    Setting ``DI_SKILLS_DIR`` short-circuits the package-walk source
...
def test_install_with_no_source_succeeds_empty(
    ...
    # Point DI_SKILLS_DIR at a non-existent path — source resolution
...
def test_install_ignores_non_skill_entries(
    ...
    (source / "di-incomplete").mkdir()  # missing SKILL.md
    _make_skill(source, "di-valid")
...
def test_install_creates_symlinks_under_both_targets(
    ...
    claude_link = home / ".claude" / "skills" / "di-shared"
    codex_link = home / ".codex" / "skills" / "di-shared"
...
def test_install_replaces_broken_symlink(
    ...
    assert dead.is_symlink() and dead.resolve() == (source / "di-shared").resolve()
...
def test_install_rewrites_stale_source_symlink(
    ...
    skill_new = _make_skill(source, "di-shared")
    stale_target = source / "di-old-name"
    stale_target.mkdir()
    (stale_target / "SKILL.md").write_text("# old")
    claude_skills = home / ".claude" / "skills"
    claude_skills.mkdir(parents=True)
    link = claude_skills / "di-shared"
...
def test_install_aborts_on_real_directory_conflict(
    ...
    claude_skills = home / ".claude" / "skills"
    claude_skills.mkdir(parents=True)
    user_owned = claude_skills / "di-shared"
    user_owned.mkdir()
    ...
    assert not (home / ".codex" / "skills" / "di-shared").exists()

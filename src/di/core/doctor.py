...
See docs/specs/2026-05-15-di-cli-architecture.md § Commands (doctor).
"""
...
def _check_source(source: Path | None) -> Check:
    ...
    if source is None:
        return Check(
            ...
            detail={"path": None},
            hint=(
                "install di-cli in editable mode (`pipx install -e /path/to/di-cli`) "
                "or set DI_SKILLS_DIR to a valid skills/ directory"
            ),
        )
    return Check(
        "source",

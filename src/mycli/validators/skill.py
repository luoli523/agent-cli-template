...
SKILL_PREFIX: str = _load_skill_prefix()
...
def validate_skill(skill_dir: Path) -> Check:
    ...
    if not name.startswith(SKILL_PREFIX):
        return Check(
            check_name,
            FAIL,
            f"skill directory name must start with `{SKILL_PREFIX}` prefix; got {name!r}",
            hint=f"rename the directory to start with `{SKILL_PREFIX}`",
        )

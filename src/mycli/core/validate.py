...
from mycli.validators import validate_repo, validate_skills_root
...
def register_subparser(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "MYCLI_SKILLS_DIR or the walk-up source resolution"
        ),
    )
        ...
        if skills_root is None:
            checks.append(
                Check(
                    ...
                    hint=(
                        "pass --skills-dir, set MYCLI_SKILLS_DIR, or run from a "
...
def _resolve_skills_root(explicit: str | None) -> Path | None:
    ...
    Priority: explicit ``--skills-dir`` > ``MYCLI_SKILLS_DIR`` env >
    walk-up from ``mycli.__file__`` for a sibling ``skills/``. The walk-up
    matches install/update's own source resolution so ``mycli validate``
    and ``mycli install`` see the same skills.
    """
    if explicit:
        p = Path(explicit).resolve()
        return p if p.is_dir() else None

    env = os.environ.get("MYCLI_SKILLS_DIR")
    if env:
        p = Path(env).resolve()
        return p if p.is_dir() else None
